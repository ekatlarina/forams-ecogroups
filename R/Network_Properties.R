...
# This script also produces the raw plots for Figures 2B, 2D, and 2F of the manuscript.
...
# load data frames containing planktonic foraminiferal data from Triton database
...
# Prepare an empty data frame for storing latitudinal properties of each time bin's network analysis
...
# Prepare an empty data frame for latitudinal properties analysis
...
# Initialize list for Indian Ocean data frames filtered by 150k-year time bins from 1.8 to 3.9 Ma
timebinsPleiPli_IO = list()

# Populate a list with data subsets for each time bin
for (i in 1:length(roundagePleiPl_150K)) {
  timebinsPleiPli_IO[[i]] = Triton_Indian[which(Triton_Indian$round.150K == roundagePleiPl_150K[i]),]
}

# Prepare an empty data frame for analyzing latitudinal properties in each time bin
...
##### 7. Plot a heatmap for the Pacific Ocean ####
...
# Generate a heatmap for the Pacific Ocean
...
##### 8. Plot a heatmap for the Indian Ocean ####

# Convert time bins to factors and prepare data frame for a heatmap creation
...
# Join data with ecological properties and reshape for a heatmap
data_wide_IO <- data %>%
  left_join(forHLeco_IO, by = c("pal.lat", "timeslab")) %>%
  mutate(pal.lat = factor(pal.lat, levels = x)) %>%
  acast(pal.lat ~ timeslab, value.var = "species.specificity.index")

# Generate a heatmap for the Indian Ocean
